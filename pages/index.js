...
export default class Index extends React.Component {
                            ...
                            <AreaChart style={{ margin: "auto" }}
                                data={
                                    this.state.county && this.state.county[this.state.name] ? Object.entries(this.state.county[this.state.name].history).sort((a, b) => {
                                        ...
                                        return { "Confirmed Cases": v[1].confirmed, "Recovered": v[1].recovered, "Deaths": v[1].deaths, day: v[0] };
                                    }) : []
                                }
    ...
    async componentWillMount() {
        var data = await (await fetch("https://9a4edec9.ngrok.io/all")).json();
        let locations = {}
        Object.keys(data.confirmed.locations).map(p => {
            locations[p] = { history: {} }
            Object.keys(data.confirmed.locations[p].history).map(d => {
                locations[p].history[d] = { confirmed: data.confirmed.locations[p].history[d], deaths: data.deaths.locations[p].history[d], recovered: data.recovered.locations[p].history[d] }
            })
        })
        this.setState({
            latest: data.latest,
